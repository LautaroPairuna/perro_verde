...
$productofotos_view = NULL; // Initialize page object first

class cproductofotos_view extends cproductofotos {
	...
	// Page object name
	var $PageObjName = 'productofotos_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "productofotoslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "productofotoslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "productofotoslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($productofotos_view)) $productofotos_view = new cproductofotos_view();

// Page init
$productofotos_view->Page_Init();

// Page main
$productofotos_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$productofotos_view->Page_Render();
?>
...
var CurrentForm = fproductofotosview = new ew_Form("fproductofotosview", "view");

// Form_CustomValidate event
fproductofotosview.Form_CustomValidate = 
...
fproductofotosview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fproductofotosview.Lists["x_producto_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_producto","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"productos"};
fproductofotosview.Lists["x_producto_id"].Data = "<?php echo $productofotos_view->producto_id->LookupFilterQuery(FALSE, "view") ?>";
fproductofotosview.Lists["x_activo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fproductofotosview.Lists["x_activo"].Options = <?php echo json_encode($productofotos_view->activo->Options()) ?>;
...
<div class="ewToolbar">
<?php $productofotos_view->ExportOptions->Render("body") ?>
<?php
	foreach ($productofotos_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $productofotos_view->ShowPageHeader(); ?>
<?php
$productofotos_view->ShowMessage();
?>
<form name="fproductofotosview" id="fproductofotosview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($productofotos_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $productofotos_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="productofotos">
<input type="hidden" name="modal" value="<?php echo intval($productofotos_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($productofotos->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_productofotos_id"><?php echo $productofotos->id->FldCaption() ?></span></td>
...
<?php echo $productofotos->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($productofotos->producto_id->Visible) { // producto_id ?>
	<tr id="r_producto_id">
		<td class="col-sm-2"><span id="elh_productofotos_producto_id"><?php echo $productofotos->producto_id->FldCaption() ?></span></td>
...
<?php echo $productofotos->producto_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($productofotos->epigrafe->Visible) { // epigrafe ?>
	<tr id="r_epigrafe">
		<td class="col-sm-2"><span id="elh_productofotos_epigrafe"><?php echo $productofotos->epigrafe->FldCaption() ?></span></td>
...
<?php echo $productofotos->epigrafe->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($productofotos->foto->Visible) { // foto ?>
	<tr id="r_foto">
		<td class="col-sm-2"><span id="elh_productofotos_foto"><?php echo $productofotos->foto->FldCaption() ?></span></td>
...
<?php echo ew_GetFileViewTag($productofotos->foto, $productofotos->foto->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($productofotos->orden->Visible) { // orden ?>
	<tr id="r_orden">
		<td class="col-sm-2"><span id="elh_productofotos_orden"><?php echo $productofotos->orden->FldCaption() ?></span></td>
...
<?php echo $productofotos->orden->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($productofotos->activo->Visible) { // activo ?>
	<tr id="r_activo">
		<td class="col-sm-2"><span id="elh_productofotos_activo"><?php echo $productofotos->activo->FldCaption() ?></span></td>
...
<?php echo $productofotos->activo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fproductofotosview.Init();
</script>
<?php
$productofotos_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$productofotos_view->Page_Terminate();
?>
