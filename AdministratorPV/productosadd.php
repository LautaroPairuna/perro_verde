...
class cproductos_add extends cproductos {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "productosview.php")
	...
	function LoadDefaultValues() {
		...
		$this->producto->CurrentValue = NULL;
		$this->producto->OldValue = $this->producto->CurrentValue;
		$this->marca_id->CurrentValue = NULL;
		$this->marca_id->OldValue = $this->marca_id->CurrentValue;
		$this->rubro_id->CurrentValue = NULL;
		$this->rubro_id->OldValue = $this->rubro_id->CurrentValue;
		$this->moneda_id->CurrentValue = NULL;
		$this->moneda_id->OldValue = $this->moneda_id->CurrentValue;
		...
		$this->destacado->CurrentValue = 0;
		$this->visitas->CurrentValue = 0;
	...
	function NewRow() {
		...
		$row['producto'] = $this->producto->CurrentValue;
		$row['marca_id'] = $this->marca_id->CurrentValue;
		$row['rubro_id'] = $this->rubro_id->CurrentValue;
		$row['moneda_id'] = $this->moneda_id->CurrentValue;
		...
		$row['destacado'] = $this->destacado->CurrentValue;
		$row['visitas'] = $this->visitas->CurrentValue;
	...
	function ValidateForm() {
		...
		if (!$this->visitas->FldIsDetailKey && !is_null($this->visitas->FormValue) && $this->visitas->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->visitas->FldCaption(), $this->visitas->ReqErrMsg));
		}
		...
		if (in_array("productoespecificaciones", $DetailTblVar) && $GLOBALS["productoespecificaciones"]->DetailAdd) {
		...
		if (in_array("productofotos", $DetailTblVar) && $GLOBALS["productofotos"]->DetailAdd) {
		...
		if (in_array("productoversiones", $DetailTblVar) && $GLOBALS["productoversiones"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// marca_id
		$this->marca_id->SetDbValueDef($rsnew, $this->marca_id->CurrentValue, 0, FALSE);

		// rubro_id
		$this->rubro_id->SetDbValueDef($rsnew, $this->rubro_id->CurrentValue, 0, FALSE);

		// moneda_id
		$this->moneda_id->SetDbValueDef($rsnew, $this->moneda_id->CurrentValue, 0, FALSE);
		...
		// destacado
		$this->destacado->SetDbValueDef($rsnew, $this->destacado->CurrentValue, 0, strval($this->destacado->CurrentValue) == "");

		// visitas
		$this->visitas->SetDbValueDef($rsnew, $this->visitas->CurrentValue, 0, strval($this->visitas->CurrentValue) == "");
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("productoespecificaciones", $DetailTblVar) && $GLOBALS["productoespecificaciones"]->DetailAdd) {
				$GLOBALS["productoespecificaciones"]->producto_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["productoespecificaciones_grid"])) $GLOBALS["productoespecificaciones_grid"] = new cproductoespecificaciones_grid(); // Get detail page object
				$AddRow = $GLOBALS["productoespecificaciones_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["productoespecificaciones"]->producto_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("productofotos", $DetailTblVar) && $GLOBALS["productofotos"]->DetailAdd) {
				$GLOBALS["productofotos"]->producto_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["productofotos_grid"])) $GLOBALS["productofotos_grid"] = new cproductofotos_grid(); // Get detail page object
				$AddRow = $GLOBALS["productofotos_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["productofotos"]->producto_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("productoversiones", $DetailTblVar) && $GLOBALS["productoversiones"]->DetailAdd) {
				$GLOBALS["productoversiones"]->producto_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["productoversiones_grid"])) $GLOBALS["productoversiones_grid"] = new cproductoversiones_grid(); // Get detail page object
				$AddRow = $GLOBALS["productoversiones_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["productoversiones"]->producto_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("productoespecificaciones", $DetailTblVar)) {
				...
				if ($GLOBALS["productoespecificaciones_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["productoespecificaciones_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["productoespecificaciones_grid"]->CurrentMode = "add";
					$GLOBALS["productoespecificaciones_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("productofotos", $DetailTblVar)) {
				...
				if ($GLOBALS["productofotos_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["productofotos_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["productofotos_grid"]->CurrentMode = "add";
					$GLOBALS["productofotos_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("productoversiones", $DetailTblVar)) {
				...
				if ($GLOBALS["productoversiones_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["productoversiones_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["productoversiones_grid"]->CurrentMode = "add";
					$GLOBALS["productoversiones_grid"]->CurrentAction = "gridadd";
...
var CurrentForm = fproductosadd = new ew_Form("fproductosadd", "add");
...
fproductosadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $productos->visitas->FldCaption(), $productos->visitas->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_visitas");
			if (elm && !ew_CheckInteger(elm.value))
...
fproductosadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fproductosadd.Lists["x_marca_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_marca","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cfgmarcas"};
fproductosadd.Lists["x_marca_id"].Data = "<?php echo $productos_add->marca_id->LookupFilterQuery(FALSE, "add") ?>";
fproductosadd.Lists["x_rubro_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_rubro","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cfgrubros"};
fproductosadd.Lists["x_rubro_id"].Data = "<?php echo $productos_add->rubro_id->LookupFilterQuery(FALSE, "add") ?>";
fproductosadd.Lists["x_moneda_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_moneda","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cfgmonedas"};
fproductosadd.Lists["x_moneda_id"].Data = "<?php echo $productos_add->moneda_id->LookupFilterQuery(FALSE, "add") ?>";
fproductosadd.Lists["x_destacado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fproductosadd.Lists["x_destacado"].Options = <?php echo json_encode($productos_add->destacado->Options()) ?>;
fproductosadd.Lists["x_activo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fproductosadd.Lists["x_activo"].Options = <?php echo json_encode($productos_add->activo->Options()) ?>;
...
?>
<form name="fproductosadd" id="fproductosadd" class="<?php echo $productos_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($productos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $productos_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="productos">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($productos_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($productos->producto->Visible) { // producto ?>
	<div id="r_producto" class="form-group">
		<label id="elh_productos_producto" for="x_producto" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->producto->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->producto->CellAttributes() ?>>
...
<?php if ($productos->marca_id->Visible) { // marca_id ?>
	<div id="r_marca_id" class="form-group">
		<label id="elh_productos_marca_id" for="x_marca_id" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->marca_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->marca_id->CellAttributes() ?>>
...
<?php if ($productos->rubro_id->Visible) { // rubro_id ?>
	<div id="r_rubro_id" class="form-group">
		<label id="elh_productos_rubro_id" for="x_rubro_id" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->rubro_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->rubro_id->CellAttributes() ?>>
...
<?php if ($productos->moneda_id->Visible) { // moneda_id ?>
	<div id="r_moneda_id" class="form-group">
		<label id="elh_productos_moneda_id" for="x_moneda_id" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->moneda_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->moneda_id->CellAttributes() ?>>
...
<?php if ($productos->descripcion->Visible) { // descripcion ?>
	<div id="r_descripcion" class="form-group">
		<label id="elh_productos_descripcion" for="x_descripcion" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->descripcion->FldCaption() ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->descripcion->CellAttributes() ?>>
...
<?php if ($productos->foto->Visible) { // foto ?>
	<div id="r_foto" class="form-group">
		<label id="elh_productos_foto" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->foto->FldCaption() ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->foto->CellAttributes() ?>>
...
<?php if ($productos->precio->Visible) { // precio ?>
	<div id="r_precio" class="form-group">
		<label id="elh_productos_precio" for="x_precio" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->precio->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->precio->CellAttributes() ?>>
...
<?php if ($productos->stock->Visible) { // stock ?>
	<div id="r_stock" class="form-group">
		<label id="elh_productos_stock" for="x_stock" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->stock->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->stock->CellAttributes() ?>>
...
<?php if ($productos->destacado->Visible) { // destacado ?>
	<div id="r_destacado" class="form-group">
		<label id="elh_productos_destacado" for="x_destacado" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->destacado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->destacado->CellAttributes() ?>>
...
<?php if ($productos->visitas->Visible) { // visitas ?>
	<div id="r_visitas" class="form-group">
		<label id="elh_productos_visitas" for="x_visitas" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->visitas->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->visitas->CellAttributes() ?>>
...
<?php if ($productos->activo->Visible) { // activo ?>
	<div id="r_activo" class="form-group">
		<label id="elh_productos_activo" for="x_activo" class="<?php echo $productos_add->LeftColumnClass ?>"><?php echo $productos->activo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $productos_add->RightColumnClass ?>"><div<?php echo $productos->activo->CellAttributes() ?>>
...
<?php
	if (in_array("productoespecificaciones", explode(",", $productos->getCurrentDetailTable())) && $productoespecificaciones->DetailAdd) {
?>
...
<?php } ?>
<?php
	if (in_array("productofotos", explode(",", $productos->getCurrentDetailTable())) && $productofotos->DetailAdd) {
?>
<?php if ($productos->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("productoversiones", explode(",", $productos->getCurrentDetailTable())) && $productoversiones->DetailAdd) {
?>
<?php if ($productos->getCurrentDetailTable() <> "") { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $productos_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
