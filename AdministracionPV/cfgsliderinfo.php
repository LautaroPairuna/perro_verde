...
//
// Table class for cfgslider
//
class ccfgslider extends cTable {
	var $id;
	var $titulo;
	var $thumbs;
	var $foto;
	...
	function __construct() {
		...
		$this->TableVar = 'cfgslider';
		$this->TableName = 'cfgslider';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cfgslider`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('cfgslider', 'cfgslider', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// titulo
		$this->titulo = new cField('cfgslider', 'cfgslider', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// thumbs
		$this->thumbs = new cField('cfgslider', 'cfgslider', 'x_thumbs', 'thumbs', '`thumbs`', '`thumbs`', 200, -1, FALSE, '`thumbs`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->thumbs->Sortable = TRUE; // Allow sort
		$this->fields['thumbs'] = &$this->thumbs;

		// foto
		$this->foto = new cField('cfgslider', 'cfgslider', 'x_foto', 'foto', '`foto`', '`foto`', 200, -1, TRUE, '`foto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->foto->Sortable = TRUE; // Allow sort
		$this->fields['foto'] = &$this->foto;

		// orden
		$this->orden = new cField('cfgslider', 'cfgslider', 'x_orden', 'orden', '`orden`', '`orden`', 3, -1, FALSE, '`orden`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// activo
		$this->activo = new cField('cfgslider', 'cfgslider', 'x_activo', 'activo', '`activo`', '`activo`', 16, -1, FALSE, '`activo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cfgslider`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`orden` ASC,`id` ASC";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "cfgslideredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "cfgslideradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cfgsliderview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cfgsliderview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cfgslideradd.php?" . $this->UrlParm($parm);
		else
			$url = "cfgslideradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cfgslideredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cfgslideradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cfgsliderdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->thumbs->setDbValue($rs->fields('thumbs'));
	...
	function RenderEditRow() {
		...
		$this->thumbs->EditValue = $this->thumbs->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->thumbs->Exportable) $Doc->ExportCaption($this->thumbs);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->thumbs->Exportable) $Doc->ExportField($this->thumbs);
