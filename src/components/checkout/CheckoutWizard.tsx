...
interface MercadoPagoConstructor {
  new (key: string, opts: { locale: string }): MercadoPagoInstance;
}
declare global {
  interface Window {
...
function Stepper({ step }: { step: number }) {
  ...
  const icons = [
    <MapPin size={20} key="step1" />,
    <CreditCard size={20} key="step2" />,
    <CheckCircle2 size={20} key="step3" />,
    <CheckCircle2 size={20} key="step4" />,
  ];
...
export default function CheckoutWizard() {
  ...
  // Cargar shipping desde localStorage
  useEffect(() => {
  ...
  // Avanza del paso 1 tras validar
  ...
  const submitOrder = async () => {
    ...
    const payload: CreatePedidoDTO & { last4?: string; payment_method_id?: string } = {
      datos: cart.map((i) => ({
        id: Number(i.id),
        name: i.name,
        price: i.price,
        ...
        <div className="w-full max-w-xl bg-white shadow-lg rounded-2xl p-6 space-y-6">
          ...
          {step === 1 && (
            <div className="space-y-4">
              {(Object.keys(shipping) as (keyof ShippingData)[]).map((field) => (
                <div key={field}>
            ...
            <div className="space-y-4">
              ...
              {paymentMethod === 'transferencia' && (
                <div className="space-y-4">
                  <p>
                    CBU:{' '}
                    <code>{process.env.NEXT_PUBLIC_CBU_EMPRESA}</code>
                  </p>
                  <input
                    placeholder="Ref. transferencia"
                    value={transferenciaRef}
                    onChange={(e) =>
                      dispatch({ type: 'SET_REF', payload: e.target.value })
                    }
                    className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-green-300"
                  />
                  <button
